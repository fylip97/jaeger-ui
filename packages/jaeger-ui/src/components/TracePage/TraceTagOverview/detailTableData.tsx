...
export const DetailTableData = (props: any) => {
  return (
    <tr className="DetailTraceTableTR1" style={{ background: props.searchColor, borderColor: props.searchColor }}>
      <td className="DetailTraceTableChildTD" onClick= {props.secondTagDropdownTitle==="sql" ? ()=> props.togglePopup(props.name): undefined }><label className="serviceBorder" style={{ borderColor: props.color }}>{props.name}</label></td>
